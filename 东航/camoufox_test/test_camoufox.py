"""
东航 Camoufox WAF 绕过测试
用法: python test_camoufox.py [出发] [到达] [日期]
示例: python test_camoufox.py 成都 广州 20250628
"""
...
SIGN_JS = SD / "sign.js"
CAMOUFOX_BRIDGE = Path(__file__).parent / "camoufox_bridge.py"
...
def encrypt(payload):
    """通过 sign.js 加密 payload → {"req": "base64..."}"""
    p = subprocess.Popen(
...
def call_camoufox(enc_req_str):
    """调用 camoufox_bridge.py，传入加密串"""
    t0 = time.time()
    r = subprocess.run(
        [sys.executable, str(CAMOUFOX_BRIDGE), enc_req_str],
        capture_output=True,
        text=True,
        timeout=180,
        cwd=str(SD),
...
if __name__ == "__main__":
    ...
    print(f"CEAIR (Camoufox test): {dn}({dep}) → {an}({arr}) {date}", file=sys.stderr)
    ...
    # 2. Camoufox API 调用
    print("[1/2] Camoufox API...", file=sys.stderr)
    enc_response = call_camoufox(enc["req"])
