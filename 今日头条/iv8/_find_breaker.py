...
document.referrer = '';
var _dc='';
Object.defineProperty(document, 'cookie', { get: function() { return _dc; }, set: function(v) { if (v) _dc = _dc ? _dc + '; ' + v : v; }, configurable: true });
document.writeln = function(html) {};
window._remoteModules = {};
window.MutationObserver = function() { this.observe = function() {}; this.disconnect = function() {}; };
window.PerformanceObserver = function() { this.observe = function() {}; this.disconnect = function() {}; };
navigator.plugins = [{ name: 'Chrome PDF Plugin', filename: 'internal-pdf-viewer' }];
navigator.mimeTypes = [{ type: 'application/pdf', suffixes: 'pdf' }];
if (!navigator.languages) navigator.languages = ['zh-CN', 'zh'];
Object.defineProperty(navigator, 'userAgentData', { get: function() { return { platform: 'Windows', brands: [{ brand: 'Chromium', version: '143' }], mobile: false }; }, configurable: true });
window.Request = function(input) { this.url = typeof input === 'string' ? input : (input && input.url); this.method = 'GET'; };
...
window.Response.prototype.json = function() { return Promise.resolve(JSON.parse(this.body || '{}')); };
...
window.XMLHttpRequest.prototype.send = function() { this.readyState = 4; this.status = 200; };
...
def test(label, extra):
    ctx = iv8.JSContext(environment=env)
    ctx.__enter__()
    ctx.eval(BASIC)
    ...
    try:
        ...
        ok = "OK" if r == "object" else "BREAKS"
        print(f"  {label:35s} -> {r:10s} {ok}")
        return r
...
print("=== Individual Group 8 items ===")
...
g8 = [
    ...
    ("URL", "var _U = window.URL || {}; _U.createObjectURL = function() { return 'blob:mock'; }; _U.revokeObjectURL = function() {}; window.URL = _U;"),
    ...
    ("wheelx+version", "window.onwheelx = { _Ax: '0X21' }; window._sdkGlueVersionMap = { sdkGlueVersion: '1.0.0.55', bdmsVersion: '1.0.1.7', captchaVersion: '4.0.2' };"),
...
for name, code in g8:
    all_g8_code += code
    test(f"  +{name}", all_g8_code)

# Test baseline + all G8 at once
test("BASELINE + ALL G8", all_g8_code)
