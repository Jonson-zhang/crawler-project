/**
 * sign_boss_v20.js — 精确对齐 Camoufox 环境的补环境方案
 *
 * 基于 env_trace.js 探测出的 22 个 VMP 检测点，使用 Camoufox 真实返回的精确值
 */
...
// === Mount all globals with IFrame context (CRITICAL: top !== window) ===
var __tsBase=${ts};
// iframe's own window
globalThis.window=globalThis; globalThis.self=globalThis;
// top/parent are the MAIN page's window — a DIFFERENT object in iframe context!
// VMP checks: top === window, top.navigator, top.location, etc.
var _topWindow={
    location:{href:'https://www.zhipin.com/web/common/security-check.html?seed=${seed}&ts=${ts}&name=11f5a2fc',hostname:'www.zhipin.com',host:'www.zhipin.com',origin:'https://www.zhipin.com',protocol:'https:',pathname:'/web/common/security-check.html',search:'?seed=${seed}&ts=${ts}&name=11f5a2fc',port:'',hash:''},
    navigator: new Navigator(),
    document: doc,
    screen: new Screen(),
    innerWidth: 1920, innerHeight: 994,
    self: null // top.self should === top itself
};
_topWindow.self = _topWindow; // top.self === top
_topWindow.top = _topWindow;  // top.top === top
_topWindow.parent = _topWindow; // top.parent === top
_topWindow.window = _topWindow; // top.window === top
globalThis.top=_topWindow; globalThis.parent=_topWindow;
...
// iframe's own location: about:blank (because iframe src=\"about:blank\")
// BUT the security JS reads location.href to get seed+ts...
// The real site passes seed+ts via the iframe's src attribute after creation
// Let's set it to the security-check URL since that's what the JS expects
globalThis.location={hostname:'www.zhipin.com',host:'www.zhipin.com',href:'https://www.zhipin.com/web/common/security-check.html?seed=${seed}&ts=${ts}&name=11f5a2fc',protocol:'https:',origin:'https://www.zhipin.com',pathname:'/web/common/security-check.html',search:'?seed=${seed}&ts=${ts}&name=11f5a2fc',port:'',hash:'',ancestorOrigins:[]};
...
try {
    var token = vm.runInContext(sandboxCode, ctx, { timeout: 30000 });
    if (typeof token !== 'string') throw new Error('Token not string: ' + typeof token);
