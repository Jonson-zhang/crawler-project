/**
 * Full deterministic test - hooks ALL possible randomness sources
 */
var vm = require('vm');
var fs = require('fs');
...
HTMLEl.prototype.offsetWidth=1280;HTMLEl.prototype.style={};HTMLEl.prototype.appendChild=mf('appendChild');
HTMLEl.prototype.setAttribute=mf('setAttribute');HTMLEl.prototype.getAttribute=function(){return null};sn(HTMLEl.prototype.getAttribute,'getAttribute');
HTMLEl.prototype.getBoundingClientRect=function(){return{x:0,y:0,width:0,height:0,top:0,left:0,right:0,bottom:0}};sn(HTMLEl.prototype.getBoundingClientRect,'getBoundingClientRect');sn(HTMLEl,'HTMLElement');
...
var nav=new Nav_();nav.userAgent='Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:135.0) Gecko/20100101 Firefox/135.0';
...
nav.vendor='';nav.vendorSub='';nav.productSub='20100101';nav.language='en-US';nav.languages=['en-US','en'];
...
p[0]=m0;p[1]=m1;pls[i]=p;});nav.plugins=pls;
...
var doc=new Doc_();doc.createElement=function(tag){if(tag==='iframe'){var f=new HTMLIFrameEl();f.style={};f.setAttribute=mf('setAttribute');f.getAttribute=function(){return null};f.contentWindow=null;return f}if(tag==='canvas')return new HTMLCanvasEl();if(tag==='script'){var s=new HTMLScriptEl();s.src='';s.setAttribute=mf('setAttribute');return s}return new HTMLEl()};sn(doc.createElement,'createElement');
...
doc.addEventListener=mf('addEventListener');doc.hidden=false;doc.readyState='complete';doc.characterSet='UTF-8';
doc.visibilityState='visible';doc.title='BOSS直聘';doc.referrer='';doc.all=undefined;
...
var sandbox = {
    Object, Array, Function, String, Number, Boolean, Date: _FakeDate, Math: _FixedMath,
...
process.stdout.write('RUN2: ' + token2 + '\n');
process.stderr.write('IDENTICAL: ' + (token1 === token2) + '\n');
process.stderr.write('LEN: ' + token1.length + '\n');
process.stderr.write('PREFIX: ' + token1.substring(0, 60) + '\n');
process.stderr.write('Access log (' + _accessLog.length + '): ' + _accessLog.slice(0, 30).join(', ') + '\n');
