/**
 * Test script - corrected env with browser-accurate values
 * Usage: node config/test_env_fix.js
 */
var vm = require('vm');
var fs = require('fs');
...
// ===== NAVIGATOR with BROWSER-ACCURATE values =====
var nav = new Nav_();
...
Object.defineProperty(doc, 'cookie', {
    get: function() { return _docCookie || ('__a=94843196.1782478489..1782478489.1.1.1.1;__c=1782478489;__g=-'); },
    set: function(v) { _docCookie = v; },
    configurable: true, enumerable: true
});

// ===== Location / Screen / History - BROWSER ACCURATE =====
var loc = new Loc_();
loc.href = 'https://www.zhipin.com/web/geek/jobs?query=python&city=101010100';
...
try {
    ...
    if (typeof sandbox.ABC !== 'undefined') {
        // Test 1: Original seed from first browser session
        console.log('=== Test 1: First seed ===');
        var seed1 = 'VsbTBCOID71h+OzSxBLPKa6ThkqrBFYaqfGa+QWt9qQ=';
        var ts1 = 1782478485106;
        var token1 = new sandbox.ABC().z(seed1, ts1);
        console.log('LEN: ' + token1.length);
        console.log('TOKEN: ' + token1);
        var browser1 = '';
        var match1 = 0;
        for (var i = 0; i < token1.length && i < browser1.length; i++) {
            if (token1[i] === browser1[i]) match1++; else break;
        }
        console.log('MATCH: ' + match1 + ' chars (first mismatch at pos ' + match1 + ')');

        // Test 2: Second seed (fresh from browser)
        console.log('\n=== Test 2: Fresh seed ===');
        var seed2 = 'VsbTBCOID71h+OzSxBLPKQnUae9Hvp8BMuN1pA1kTUE=';
        var ts2 = 1782478606552;
        var token2 = new sandbox.ABC().z(seed2, ts2);
        console.log('LEN: ' + token2.length);
        console.log('TOKEN: ' + token2);
    } else {
        console.log('ERROR: ABC not defined');
    }
} catch(e) {
