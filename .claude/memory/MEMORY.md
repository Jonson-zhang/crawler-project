...
> ⚠️ **规则 0（最高优先级）：禁止 jsdom。** 每次逆向启动时首先检查。jsdom 有三个致命缺陷：易被检测、无法 Hook 早期代码（source patch 语法错误）、产出不可部署。国家医保局 12 个 jsdom 脚本全部白费。详见 [[stop-loss-rules#规则0]]。**如有 DOM 依赖需求，用 iv8 而非 jsdom。**
