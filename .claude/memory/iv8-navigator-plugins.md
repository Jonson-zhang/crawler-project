...
2026-06-27 小红书 iv8 迁移：VMP 能正常加载（DOM stubs + IIFE 作用域 + forEach 闭包都修了）但 `mnsv2` 仍是 `undefined`。逐步排查所有环境差异时发现 `navigator.plugins` 在 iv8 中是真实值（length=5），env.js 中是空数组（length=0）。显式覆盖后 VMP 输出正常。
