/**
 * 米画师 (mihuashi.com) API 签名器 — Node.js WASM 加载
 *
 * 补全 42 个 wbg 导入函数。参数签名与 wasm-objdump Type section 严格对齐。
 * signtool_new() 读取 DOM 特征 (favicon+keywords+crypto 随机数) 派生密钥，
 * signtool_sign() 对 URL 路径+时间戳生成 88 字符签名。
 *
 * 用法: node sign.js <url_path> <timestamp>
 * 输出: 88 字符签名字符串
 */
...
// ============================================================
// 浏览器环境 — fakeWindow 必须直接挂载 crypto/navigator/document
// 因为 WASM 通过 __wbg_crypto(selfRef) → tblGet(selfRef).crypto 链式访问
// ============================================================
...
var fakeFaviconLink = {
    ...
    getAttribute: function (n) { return n === 'href' ? DEFAULT_FAVICON : null; },
};
var fakeMetaKeywords = {
    content: DEFAULT_KEYWORDS,
    getAttribute: function (n) { return n === 'content' ? DEFAULT_KEYWORDS : null; },
};
var fakeDocument = {
...
// ============================================================
// 42 个 wbg 导入 — 参数签名与 WASM Type section 严格对齐
// 生成: wasm-objdump -x mhs_fe_sign_bg.wasm | node tools/gen_stub_template.js
// ============================================================
var wbg = {
    // sig=1 (i32,i32)→i32
    ...
    // sig=2 (i32)→i32 — 通过 selfRef 链获取 self.crypto
    ...
    // sig=3 (i32,i32)→void — ptrOut, valRef
    ...
    // sig=0 (i32,i32,i32)→i32 — docRef, strPtr, strLen
...
if (!urlPath || isNaN(timestamp)) {
    console.error('Usage: node sign.js <url_path> <timestamp>');
    process.exit(1);
}
