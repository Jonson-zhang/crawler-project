...
原理:
  execjs 调用 gov_nhsa_encrypt.js (webpack提取版) 生成 SM4/SM2/SHA256 加密请求
...
# 通过 subprocess 直接调用 Node.js (避免 execjs 的 GBK 编码问题)
import subprocess

JS_BOOTSTRAP = ENCRYPT_JS.read_text("utf-8")

def js_call(func_name, *args):
    """调用 gov_nhsa_encrypt.js 中的函数"""
    args_json = json.dumps(list(args), ensure_ascii=False)
    node_script = f"""
{JS_BOOTSTRAP}
var _result = {func_name}.apply(null, JSON.parse({json.dumps(args_json)}));
        ...
        for line in r.stdout.strip().split("\n"):
            ...
            if line.startswith("{"):
                try:
                    ...
                    pass
        raise RuntimeError(f"JS call failed: {r.stderr[:200] if r.stderr else r.stdout[:200]}")
    finally:
        try: tmp_file.unlink()
        except: pass
...
class NhsaClient:
    ...
    def search(self, medins_name="", regn_code="110000", page_num=1, page_size=10):
        """查询医疗机构"""
        params = {
            "addr": "",
            "regnCode": regn_code,
        ...
        # 调用 JS 加密
        print(f"[Nhsa] Encrypting: medinsName='{medins_name}', regnCode={regn_code}", file=sys.stderr)
        encrypted = js_call("EncryptedData", params,
                            "/nthl/api/CommQuery/queryFixedHospital")

        headers = {**self.BASE_HEADERS}
        headers["x-tif-timestamp"] = str(encrypted["headers"]["x-tif-timestamp"])
        headers["x-tif-nonce"] = encrypted["headers"]["x-tif-nonce"]
        headers["x-tif-signature"] = encrypted["headers"]["x-tif-signature"]
        headers["x-tif-paasid"] = encrypted["headers"].get("x-tif-paasid", "undefined")
        headers["contentType"] = encrypted["headers"].get("contentType", "application/x-www-form-urlencoded")

        body = json.dumps({"data": encrypted["data"]}, separators=(",", ":"))

        print(f"[Nhsa] x-tif-sig: {headers['x-tif-signature'][:16]}...", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("keyword", nargs="?", default="",
                        help="医疗机构名称 (留空查询全部)")
    parser.add_argument("--regn", "-r", default="110000",
                        help="行政区划代码 (默认110000=北京市)")
